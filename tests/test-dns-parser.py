  ...
  def testaaaa_nok(self):
    for addr in [':::', '1.2.3.4', '::1::']:
      self.assertRaises(autoreg.dns.parser.ParseError, self.p.parseline,
                        ' AAAA %s' % addr)
  def testns_ok(self):
    self._test_type_val('NS', ['ns.eu.org.', 'signal', 'a.root-servers.net.'],
                        force_upper=True)
  def testptr_ok(self):
    self._test_type_val('PTR', ['ns.eu.org.', 'signal', 'a.root-servers.net.'])
  def testcname_ok(self):
    self._test_type_val('CNAME',
                        ['ns.eu.org.', 'signal', 'a.root-servers.net.'],
                        force_upper=True)
  def testmx_ok(self):
    self._test_type_val('MX', ['10 eu.org.', '0 blablabla.org.', '255 signal',
                               '20 ns.eu.org.'],
